...
## I want Account Guard out of the way, now

Account Guard changes exactly one setting outside its own namespace, and it is **global**:

```jsonc
// %APPDATA%\Code\User\settings.json
"claudeCode.claudeProcessWrapper": "%LOCALAPPDATA%\\ClaudeAccountGuard\\wrapper\\claude-account-guard-wrapper.exe"
```

Three escape hatches, in increasing order of finality:

1. **Bypass without changing anything.** Set the environment variable `CLAUDE_ACCOUNT_GUARD_DISABLE=1` (for one shell: `$env:CLAUDE_ACCOUNT_GUARD_DISABLE = "1"`; permanently: `setx CLAUDE_ACCOUNT_GUARD_DISABLE 1`, then restart VS Code). The wrapper forwards straight to Claude Code with your default account. No per-workspace account is applied while this is set.
2. **Disconnect from the UI.** Run **Claude Account Guard: Disconnect From Claude Code**, then choose **Reload Window**. It clears `claudeCode.claudeProcessWrapper`, or restores a third-party wrapper Account Guard chained, and says which of the two it did. Every workspace goes back to your default Claude account. Accounts, bindings, and collected usage are kept, so reconnecting later restores the previous behaviour. Account Guard will not silently reconfigure the setting afterwards — disconnecting is remembered.
3. **Remove everything.** Run **Claude Account Guard: Remove All Account Guard Data**. It disconnects as above, restores any status-line command it chained into an account's Claude `settings.json`, removes the terminal `CLAUDE_CONFIG_DIR` it set for this workspace, deletes its wrapper files and its `binding-cache.json`, and deletes all accounts, bindings, and local usage under `%LOCALAPPDATA%\ClaudeAccountGuard`. Reload the window afterwards, then uninstall the extension if you are done.

A workspace can also switch the guard off for its own terminals: `terminal.integrated.env.windows` is workspace-scoped, so a checked-in `.vscode/settings.json` setting `CLAUDE_ACCOUNT_GUARD_DISABLE` to `1` disables the guard for terminals opened in that repository. That is the same mechanism Account Guard's opt-in terminal binding uses, and it is worth knowing before cloning somebody else's repository.
...
4. Delete `%LOCALAPPDATA%\ClaudeAccountGuard`.

Reinstalling Account Guard also repairs this: at activation it checks whether the configured wrapper still exists on disk, reinstalls it or clears the setting, and tells you which. It never leaves Claude Code pointed at a missing wrapper.

## Nothing global is written until it is needed

Account Guard does not touch `claudeCode.claudeProcessWrapper` until you give a workspace its own Claude account, or turn on local usage collection — and then it asks once, naming the setting and how to undo it. Declining is remembered across windows, and the binding is saved but reported as not applied. To connect later, run **Claude Account Guard: Connect to Claude Code**. If `claudeAccountGuard.wrapper.autoConfigure` is `false`, Account Guard stays read-only and will not ask at all; per-workspace accounts then do nothing.
...
- **"Set to use X, but not applied yet".** Claude Code is not launching through Account Guard. Run **Connect to Claude Code**.
...
- **`CLAUDE_ACCOUNT_GUARD_DISABLE=1` is set** in your environment, which bypasses everything by design.
- **`claudeAccountGuard.defaultLockMode` is `off`.** The choice is recorded and deliberately not applied.
- **A `claude` you started in a terminal.** The wrapper only covers launches made by the Claude Code extension. When binding a workspace, accept **Also use this account for `claude` in this workspace's terminals** to have Account Guard write `CLAUDE_CONFIG_DIR` into this workspace's `terminal.integrated.env.windows`. If that setting cannot be written at workspace scope in your setup, Account Guard says so and leaves it to you; set `CLAUDE_CONFIG_DIR` in the terminal yourself. Terminals already open keep their old environment.
...
- **Update the expected identity.** Click the Claude status bar item, or run **Claude Account Guard: Update This Workspace's Expected Claude Identity** (it is also offered from the diagnostics report). It shows the identity that was recorded and the one that answers now, and records the new one if that is what you intended.
- **Only warn here.** The same prompt offers switching this workspace to `warn`, which reports a mismatch in the status bar and never stops a launch. `claudeAccountGuard.defaultLockMode` sets the default for new bindings.
...
- Account Guard cannot detect that such a directory has been signed into a different Claude account, so `enforce` behaves like `warn` for it.
- Your **default** account is exempt: Account Guard probes it without setting the variable, so the real email and organization are recorded and drift detection works for it.
...
1. **`claudeAccountGuard.telemetry.enabled` is off.** Nothing is collected.
2. **The account in play here is not one Account Guard knows about.** Usually this is your default account in a workspace with no account of its own. Nothing can be attributed to it. Fix: **Track the Default Claude Account**, or give the workspace one of your accounts.
...
5. **Claude Code does not launch through Account Guard.** Quota snapshots still arrive, but token, cost, and tool detail arrive over OpenTelemetry that Account Guard injects at launch.
6. **You already configure your own OTLP exporter.** `OTEL_EXPORTER_OTLP_*` is set, and Account Guard deliberately does not redirect Claude's telemetry away from it.
...
Account Guard never silently overwrites it. When a binding or local usage needs the integration, it shows the other wrapper's path and offers **Chain And Connect**, which selects the workspace's account first and then runs your wrapper. A chained wrapper inherits the environment Account Guard prepared — including the bound `CLAUDE_CONFIG_DIR` and, when usage collection is on, the local collector's bearer token — so only chain a wrapper you trust with those. Declining leaves your wrapper untouched. **Disconnect From Claude Code** restores it. Diagnostics shows both redacted paths and any unresolved conflict.

## Status lines

The status-line bridge is per account: it is written only to `<that account's CLAUDE_CONFIG_DIR>\settings.json`. Turning on usage collection for one account never modifies another's, and never modifies the default `%USERPROFILE%\.claude` unless that directory is itself the account being tracked. An existing `statusLine` command is recorded in `<config dir>\.claude-account-guard\statusline-next.json`, run after the bridge on every status-line refresh, and restored when the account is removed from Account Guard or Account Guard data is removed.

Never delete Claude configuration directories as part of Account Guard cleanup; they belong to Claude Code and contain credentials.
